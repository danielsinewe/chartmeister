...
var chartmeister_schema_default = {
  $schema: "http://json-schema.org/draft-07/schema#",
  $id: "https://github.com/danielsinewe/chartmeister/chartmeister-schema.json",
  title: "Chartmeister File Format Schema",
  description: "JSON schema for .chartmeister files - a format for saving and loading chart data, similar to Excalidraw's .excalidraw format. Version 6 adds font size overrides for value labels, category labels, axis labels, and legend labels for typography harmonization.",
  version: 6,
  type: "object",
...
// src/index.ts
var schemaVersion = chartmeister_schema_default.version ?? -1;
function validate(file) {
  const ajv = new import_ajv.Ajv({ allErrors: true, strict: false });
  const validateFn = ajv.compile(chartmeister_schema_default);
  const valid = validateFn(file);
  return { valid: Boolean(valid), errors: validateFn.errors ?? void 0 };
}
...
export {
  migrate,
  chartmeister_schema_default as schema,
  schemaVersion,
  validate
};
